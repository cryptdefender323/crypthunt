...
The installer path is implemented in `packages/omop-codex/src/telemetry/`. The runtime plugin path is implemented in `packages/omop-codex/plugin/components/telemetry/`, which is copied into the `code-yeongyu/lazycodex` marketplace repository under `plugins/omo/components/telemetry/`.
