...
Complete reference for CryptHunter plugin configuration. During the rename transition, the runtime recognizes both `oh-my-open-pentest.json[c]` and legacy `oh-my-open-pentest.json[c]` files.
...
```json
{
  "$schema": "https://raw.githubusercontent.com/cryptdefender323/crypthunter/dev/assets/crypthunter.schema.json"
}
```
