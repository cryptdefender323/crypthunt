...
- Never paste agent-facing system prompts here. Those live in [`packages/omop-opencode/src/agents/`](../packages/omop-opencode/src/agents/) or [`packages/omop-opencode/src/features/builtin-skills/`](../packages/omop-opencode/src/features/builtin-skills/).
- Never document changing config keys without also updating [`packages/omop-opencode/src/config/schema/`](../packages/omop-opencode/src/config/schema/) and re-running `bun run build:schema`.
