...
Project-scope OpenCode configuration: 5 skills and 5 slash commands committed alongside the source. Picked up by [`packages/omop-opencode/src/features/opencode-skill-loader/`](../packages/omop-opencode/src/features/opencode-skill-loader/) and the slash-command discovery pipeline.
...
- **Skill YAML frontmatter is mandatory.** [`opencode-skill-loader`](../packages/omop-opencode/src/features/opencode-skill-loader/) rejects skills without `name` + `description`.
...
- Never duplicate a built-in skill from `packages/omop-opencode/src/features/builtin-skills/` here without a clear reason.
