#!/usr/bin/env bash
# Cross-harness teardown for oh-my-open-pentest dev environments.
#
...
# Safety guard: refuse to run unless we are at the oh-my-open-pentest repo root.
if ! grep -q '"oh-my-open-pentest"' "$REPO_ROOT/package.json" 2>/dev/null; then
  log "ERROR: refusing to run - $REPO_ROOT is not the oh-my-open-pentest repo root"
  exit 1
fi
