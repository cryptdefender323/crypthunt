...
describe("currentRequestContext", () => {
	it("#given project, user, and install-decision config env #when building request context #then forwards only those keys", () => {
		const context = currentRequestContext({
			LSP_TOOLS_MCP_PROJECT_CONFIG: ".opencode/lsp.json:.omop/lsp.json",
			LSP_TOOLS_MCP_USER_CONFIG: "~/.omop/lsp.json",
			LSP_TOOLS_MCP_INSTALL_DECISIONS: "~/.omop/lsp-install-decisions.json",
			PATH: "/usr/bin",
			HOME: "/home/me",
		});
