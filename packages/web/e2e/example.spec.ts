...
test.describe("Landing Page", () => {
  test("renders hero section with title and CTA", async ({ page }) => {
    ...
    // then
    await expect(page).toHaveTitle(/CryptHunter/)
...
test.describe("Manifesto Page", () => {
  ...
  test("renders CTA with GitHub link", async ({ page }) => {
    ...
    // when
    const ctaLink = page.getByRole("link", { name: /Get CryptHunter/i })

    // then
    await expect(ctaLink).toBeVisible()
    await expect(ctaLink).toHaveAttribute("href", "https://github.com/cryptdefender323/crypthunter")
  })
})
