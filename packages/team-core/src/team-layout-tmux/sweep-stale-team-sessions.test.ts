...
describe("sweepStaleTeamSessionsWith", () => {
	it("#given candidates with mix of active and stale #when sweep #then kills only sessions whose runId is not in active set", async () => {
		...
		const fixture = createFixture([
			"omop-team-11111111-1111-1111-1111-111111111111",
			"omop-team-22222222-2222-2222-2222-222222222222",
			"omop-team-33333333-3333-3333-3333-333333333333",
			"main",
			"omop-agents-123",
		])
	...
	it("#given killSession throws for one #when sweep #then continues and returns only successful kills", async () => {
		...
		fixture.killSessionMock.mockImplementation(async (sessionName: string): Promise<void> => {
			if (sessionName === "omop-team-22222222-2222-2222-2222-222222222222") {
	...
	it("#given candidate name is 'omop-team-' with empty suffix #when sweep #then skipped", async () => {
		// given
		const fixture = createFixture(["omop-team-", "omop-team-11111111-1111-1111-1111-111111111111"])
		const activeTeamRunIds = new Set<string>()
		...
		// then
		expect(fixture.killedSessionNames).toEqual(["omop-team-11111111-1111-1111-1111-111111111111"])
		expect(result).toEqual(["omop-team-11111111-1111-1111-1111-111111111111"])
	})

	it("#given new caller-session topology rolled out with no omop-team-<uuid> candidates #when sweep runs #then the result is empty and killSession is never called", async () => {
		// given
	...
	it("#given a user tmux session named like a project hash #when sweep runs #then it is preserved because only UUID-backed team sessions are eligible", async () => {
		// given
		const fixture = createFixture(["main", "omop-team-de2e", "dev-shell"])
		const activeTeamRunIds = new Set<string>()
