...
test("#given win32 install with OMOP_CODEX_GIT_BASH_PATH set #when stamping #then git_bash server env forwards the override", async (t) => {
	...
	const changed = await stampGitBashMcpEnv({
		pluginRoot,
		env: { OMOP_CODEX_GIT_BASH_PATH: "D:\\Git\\bin\\bash.exe" },
		platform: "win32",
	});

	assert.equal(changed, true);
	const parsed = JSON.parse(await readFile(join(pluginRoot, ".mcp.json"), "utf8"));
	assert.deepEqual(parsed.mcpServers.git_bash.env, { OMOP_CODEX_GIT_BASH_PATH: "D:\\Git\\bin\\bash.exe" });
...
test("#given the override already stamped #when stamping again #then nothing changes", async (t) => {
	const pluginRoot = await createPluginRoot();
	t.after(() => rm(pluginRoot, { recursive: true, force: true }));
	const input = { pluginRoot, env: { OMOP_CODEX_GIT_BASH_PATH: "D:\\Git\\bin\\bash.exe" }, platform: "win32" };
