...
describe("omop-codex posthog telemetry", () => {
  beforeEach(() => {
    mock.restore()
    clearTelemetryEnv()
  })
  ...
  it("captures omo_codex_daily_active with omop-codex platform", async () => {
    // given
    const capturedMessages: CapturedPostHogMessage[] = []
    mockPostHogNode(capturedMessages)
    process.env.POSTHOG_API_KEY = "test-api-key"
    ...
    expect(capturedMessages[0]?.properties?.platform).toBe("omop-codex")
  })
