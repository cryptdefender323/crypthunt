...
import { resolveInstallArgs } from "../../../omop-opencode/src/cli/cli-program"
import { argsToConfig } from "../../../omop-opencode/src/cli/install-validators"

describe("lazycodex install routing", () => {
  const originalInvocationName = process.env.OMOP_INVOCATION_NAME

  afterEach(() => {
    if (originalInvocationName === undefined) {
      delete process.env.OMOP_INVOCATION_NAME
    } else {
      process.env.OMOP_INVOCATION_NAME = originalInvocationName
    }
  })
  ...
  test("defaults platform to codex when invoked as lazycodex-ai", () => {
    // given
    process.env.OMOP_INVOCATION_NAME = "lazycodex-ai"
  ...
  test("leaves omo install unresolved so argsToConfig applies opencode default", () => {
    // given
    process.env.OMOP_INVOCATION_NAME = "oh-my-open-pentest"
