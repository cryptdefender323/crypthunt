...
async function createPackagedCodexRepoRoot(): Promise<string> {
  const repoRoot = await mkdtemp(join(tmpdir(), "omop-codex-project-cleanup-repo-"))
  ...
  await writeFile(
    ...
    JSON.stringify({ name: "@cerberuslabs/omop-codex-plugin", version: "0.1.0" }),
  )
...
describe("install-codex project-local cleanup", () => {
  test("#given stale project-local Codex config #when installing Codex Light #then repairs the local conflict before returning success", async () => {
    // given
    const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-home-project-cleanup-"))
    const binDir = await mkdtemp(join(tmpdir(), "omop-codex-bin-project-cleanup-"))
    const projectRoot = await mkdtemp(join(tmpdir(), "omop-codex-project-cleanup-install-"))
  ...
  test("#given only global CODEX_HOME config under a parent directory #when installing Codex Light #then project cleanup leaves it to the global config updater", async () => {
    // given
    const homeRoot = await mkdtemp(join(tmpdir(), "omop-codex-home-parent-cleanup-"))
    const codexHome = join(homeRoot, ".codex")
    const binDir = await mkdtemp(join(tmpdir(), "omop-codex-bin-parent-cleanup-"))
  ...
  test("#given project cleanup hits a filesystem edge #when installing Codex Light #then install succeeds and reports skipped cleanup", async () => {
    // given
    const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-home-project-edge-"))
    const binDir = await mkdtemp(join(tmpdir(), "omop-codex-bin-project-edge-"))
    const projectRoot = await mkdtemp(join(tmpdir(), "omop-codex-project-edge-"))
