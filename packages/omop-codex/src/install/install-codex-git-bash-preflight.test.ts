...
describe("install-codex Git Bash preflight", () => {
  test("#given Windows without Git Bash and auto install skip env #when installing Codex profile #then rejects before marketplace or config mutation", async () => {
    // given
    const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-git-bash-missing-home-"))
    const repoRoot = await mkdtemp(join(tmpdir(), "omop-codex-git-bash-missing-repo-"))
    ...
    const install = runCodexInstaller({
      ...
      gitBashResolver: () => ({
        ...
        installHint: [
          ...
          "OMOP_CODEX_GIT_BASH_PATH=C:\\path\\to\\bash.exe",
  ...
  test("#given Windows without Git Bash #when winget succeeds and resolver recovers #then install continues", async () => {
    // given
    const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-git-bash-auto-install-home-"))
    const binDir = await mkdtemp(join(tmpdir(), "omop-codex-git-bash-auto-install-bin-"))
  ...
  test("#given non-Windows install #when running installer #then winget is never called", async () => {
    // given
    const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-git-bash-no-winget-linux-home-"))
    const binDir = await mkdtemp(join(tmpdir(), "omop-codex-git-bash-no-winget-linux-bin-"))
    const runCalls: string[] = []
  ...
  test("#given Windows with Git Bash #when installing Codex profile #then proceeds and reports detected path", async () => {
    // given
    const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-git-bash-present-home-"))
    const binDir = await mkdtemp(join(tmpdir(), "omop-codex-git-bash-present-bin-"))
  ...
  test("#given Windows env override in installer options #when no custom resolver is provided #then default resolver uses it", async () => {
    // given
    const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-git-bash-env-home-"))
    const binDir = await mkdtemp(join(tmpdir(), "omop-codex-git-bash-env-bin-"))
    const gitBashPath = join(await mkdtemp(join(tmpdir(), "omop-codex-git-bash-env-")), "bash.exe")
    ...
    const result = await withBundledLspRuntimeForTest(async () => runCodexInstaller({
      ...
      platform: "win32",
      astGrepInstaller: skipAstGrepInstall,
      env: { OMOP_CODEX_GIT_BASH_PATH: gitBashPath },
      runCommand: async () => undefined,
    }))
  ...
  test("#given non-Windows install #when Git Bash resolver would fail #then installer keeps existing behavior", async () => {
    // given
    const codexHome = await mkdtemp(join(tmpdir(), "omop-codex-git-bash-linux-home-"))
    const binDir = await mkdtemp(join(tmpdir(), "omop-codex-git-bash-linux-bin-"))
