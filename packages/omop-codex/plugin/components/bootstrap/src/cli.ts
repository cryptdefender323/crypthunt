...
const TOP_LEVEL_HELP =
	"Usage:\n  omop-bootstrap hook session-start\n  omop-bootstrap worker [--codex-home <dir>] [--once] [--only <step>] [--manifest-dir <dir>]\n  omop-bootstrap download <manifest> <platform> <destination-dir>\n  omop-bootstrap help | --help | -h\n";
...
async function runDownloadCommand(args: readonly string[]): Promise<number> {
	...
	if (manifestName === undefined || platformKey === undefined || destinationDir === undefined) {
		process.stderr.write(`[omop-bootstrap] download requires <manifest> <platform> <destination-dir>\n${TOP_LEVEL_HELP}`);
		return 1;
	}
	try {
		...
		process.stderr.write(`[omop-bootstrap] download failed: ${error instanceof Error ? error.message : String(error)}\n`);
		return 1;
	}
}
...
async function runWorkerCommand(args: readonly string[]): Promise<number> {
	...
	try {
		...
		if (/flag/.test(message)) {
			process.stderr.write(`[omop-bootstrap] ${message}\n${TOP_LEVEL_HELP}`);
			return 1;
		}
		// Runtime worker failures must never surface as non-zero exits; the
		// degraded ledger in state.json is the error channel.
		process.stderr.write(`[omop-bootstrap] worker error: ${message}\n`);
		return 0;
	}
	process.stdout.write(
		result.ran ? `[omop-bootstrap] worker finished: ${result.status}\n` : `[omop-bootstrap] worker skipped: ${result.reason}\n`,
	);
	return 0;
}
...
async function main(): Promise<number> {
	...
	process.stderr.write(`[omop-bootstrap] unknown command: ${argv.join(" ")}\n${TOP_LEVEL_HELP}`);
	return 1;
}
...
if (isProcessEntry()) {
	main()
		...
		.catch((error: unknown) => {
			...
			process.stderr.write(`[omop-bootstrap] ${error instanceof Error ? error.message : String(error)}\n`);
			process.exit(0);
		});
}
