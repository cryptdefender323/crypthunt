...
function Resolve-GitBash {
	if (-not [string]::IsNullOrWhiteSpace($env:OMOP_CODEX_GIT_BASH_PATH)) {
		$envCandidate = $env:OMOP_CODEX_GIT_BASH_PATH.Trim()
...
function Initialize-GitBash {
	...
	if (($null -eq $resolution) -and ($env:OMOP_CODEX_SKIP_GIT_BASH_AUTO_INSTALL -ne "1")) {
		$winget = Get-Command winget -ErrorAction SilentlyContinue
		if ($null -ne $winget) {
	...
	if ($resolution.Source -eq "path") {
		$persisted = [Environment]::GetEnvironmentVariable("OMOP_CODEX_GIT_BASH_PATH", "User")
		if ([string]::IsNullOrWhiteSpace($persisted)) {
			[Environment]::SetEnvironmentVariable("OMOP_CODEX_GIT_BASH_PATH", $resolution.Path, "User")
			Write-BootstrapLog ("persisted OMOP_CODEX_GIT_BASH_PATH=" + $resolution.Path + " in the USER environment; restart Codex to pick it up")
		}
	}
}
