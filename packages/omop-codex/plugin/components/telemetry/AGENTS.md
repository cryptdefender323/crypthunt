...
- Constants in `src/product-identity.ts` MUST stay byte-equivalent with `packages/omop-codex/src/telemetry/product-identity.ts`. The cross-package equivalence test will fail otherwise.
