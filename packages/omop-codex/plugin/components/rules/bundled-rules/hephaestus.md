...
omop-codex auto-runs LSP diagnostics after every edit and injects the result: any reported error is blocking until resolved. You may also invoke diagnostics explicitly.

# Subagents

omop-codex bundles read-only Codex subagent roles in `CODEX_HOME/agents/`. Spawn: `multi_agent_v..spawn_agent({"message":"TASK: act as a <role>. ...","fork_context":false})`
