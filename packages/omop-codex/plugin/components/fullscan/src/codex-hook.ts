...
import { FULLSCAN_DIRECTIVE } from "./directive.js";

const FULLSCAN_CURRENT_PROMPT_PATTERN = /(?:fullscan|ulw)/i;
const FULLSCAN_DIRECTIVE_MARKER = "<fullscan-mode>";
...
export function runUserPromptSubmitHook(input: unknown): string {
	...
	return isFullscanPrompt(input.prompt) ? formatAdditionalContextOutput(FULLSCAN_DIRECTIVE) : "";
}
	...
	try {
		...
		for (const line of rawTranscript.split(/\r?\n/)) {
			...
			if (
				...
				hookSpecificOutput["additionalContext"].includes(FULLSCAN_DIRECTIVE_MARKER)
			) {
				return true;
			}
		}
...
export function isFullscanPrompt(prompt: string): boolean {
	return FULLSCAN_CURRENT_PROMPT_PATTERN.test(prompt);
}
