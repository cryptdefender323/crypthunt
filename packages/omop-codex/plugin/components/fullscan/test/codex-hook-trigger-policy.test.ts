...
import { isFullscanPrompt, runUserPromptSubmitHook } from "../src/codex-hook.js";
...
describe("codex fullscan trigger policy", () => {
	it("#given fullscan variants in current prompt #when hook runs #then emits directive", () => {
		...
		expect(prompts.map((prompt) => isFullscanPrompt(prompt))).toEqual([true, true, true]);
	});
	...
	it("#given ulw variants in current prompt #when hook runs #then emits directive", () => {
		...
		expect(prompts.map((prompt) => isFullscanPrompt(prompt))).toEqual([true, true, true, true, true]);
	});
	...
	it("#given sentence-level triggers in current prompt #when hook runs #then emits directive", () => {
		...
		expect(prompts.map((prompt) => isFullscanPrompt(prompt))).toEqual([true, true]);
	});
	...
	it("#given reported prompt contains trigger text in current prompt #when hook runs #then emits directive", () => {
		...
		expect(isFullscanPrompt(prompt)).toBe(true);
	});
	...
	it("#given prior transcript contains triggers but current prompt does not #when hook runs #then does not emit directive", () => {
		...
		// then
		expect(output).toBe("");
		expect(isFullscanPrompt(payload.prompt)).toBe(false);
	});
});
