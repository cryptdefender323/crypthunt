...
test("#given source package versions and component without hooks #when hook status messages sync #then source versions are preserved", async () => {
	...
	assert.equal(aggregateHooks.hooks.PostToolUse[0].hooks[0].statusMessage, "(OmOP) Checking Comments");
	assert.equal(aggregateHooks.hooks.PostToolUse[0].hooks[1].statusMessage, "(OmOP) Checking LSP Diagnostics");
	assert.equal(componentHooks.hooks.PostToolUse[0].hooks[0].statusMessage, "(OmOP) Checking Comments");
	assert.equal(lspHooks.hooks.PostToolUse[0].hooks[0].statusMessage, "(OmOP) Checking LSP Diagnostics");
});
