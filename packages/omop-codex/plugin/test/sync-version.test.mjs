...
async function makeTempDir() {
	return mkdtemp(join(tmpdir(), "omop-codex-sync-version-"));
}
...
async function scaffoldPlugin(repoRoot, rootVersion) {
	...
	await writeJson(join(repoRoot, "packages", "omop-codex", "package.json"), { name: "@omop/omop-codex", version: "0.1.0" });
	await writeJson(join(pluginRoot, "package.json"), { name: "@cerberuslabs/omop-codex-plugin", version: "0.1.0" });
	await writeJson(join(pluginRoot, ".codex-plugin", "plugin.json"), { name: "omop", version: "0.1.0" });
