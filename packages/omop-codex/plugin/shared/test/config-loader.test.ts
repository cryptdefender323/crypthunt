...
describe("getCodexOmoConfig", () => {
	it("#given base and codex SOT blocks #when loading config #then returns the codex-merged effective config", () => {
		// given
		const homeDir = createTemporaryDirectory("omop-codex-shared-")
		const cwd = createTemporaryDirectory("omop-codex-project-")
		writeOmoConfig(
			homeDir,
			JSON.stringify({
	...
	it("#given legacy env override and SOT value #when loading config #then env wins over the SOT", () => {
		// given
		const homeDir = createTemporaryDirectory("omop-codex-shared-env-")
		const cwd = createTemporaryDirectory("omop-codex-project-env-")
	...
	it("#given no SOT files #when loading config #then returns built-in defaults and missing global source", () => {
		// given
		const homeDir = createTemporaryDirectory("omop-codex-shared-defaults-")
		const cwd = createTemporaryDirectory("omop-codex-project-defaults-")
	...
	it("#given codex-unsupported codegraph setting #when loading config #then returns loader warnings", () => {
		// given
		const homeDir = createTemporaryDirectory("omop-codex-shared-warnings-")
		const cwd = createTemporaryDirectory("omop-codex-project-warnings-")
