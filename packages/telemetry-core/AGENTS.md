...
- **Opt-out env matrix:** `${PREFIX}_DISABLE_POSTHOG` (truthy `./true/yes`) OR `${PREFIX}_SEND_ANONYMOUS_TELEMETRY` (opt-out `0/false/no/yes`); global `OMOP_` prefix also checked.
