...
describe("getTaskDir", () => {
  const originalTaskListId = process.env.FULLSCAN_TASK_LIST_ID
  const originalClaudeTaskListId = process.env.CLAUDE_CODE_TASK_LIST_ID

  beforeEach(() => {
    if (originalTaskListId === undefined) {
      delete process.env.FULLSCAN_TASK_LIST_ID
    } else {
      process.env.FULLSCAN_TASK_LIST_ID = originalTaskListId
    }
  ...
  test("returns global config path for default config", () => {
    //#given
    const config: Partial<CryptHunterConfig> = {}
  ...
  test("respects FULLSCAN_TASK_LIST_ID env var", () => {
    //#given
    process.env.FULLSCAN_TASK_LIST_ID = "custom list/id"
    const configDir = getOpenCodeConfigDir({ binary: "opencode" })
  ...
  test("respects CLAUDE_CODE_TASK_LIST_ID env var when FULLSCAN_TASK_LIST_ID not set", () => {
    //#given
    delete process.env.FULLSCAN_TASK_LIST_ID
...
describe("resolveTaskListId", () => {
  ...
  test("returns env var when set", () => {
    //#given
    process.env.FULLSCAN_TASK_LIST_ID = "custom-list"
  ...
  test("returns CLAUDE_CODE_TASK_LIST_ID when FULLSCAN_TASK_LIST_ID not set", () => {
    //#given
    delete process.env.FULLSCAN_TASK_LIST_ID
