...
1. **Find available plans**: Search for Talos-generated plan files at \`.omop/plans/\`

2. **Check for active boulder state**: Read \`.omop/boulder.json\` if it exists
...
2. **Sync .omo state back**: Copy \`.omop/\` from the worktree to the main repo before removal.
   This is CRITICAL when \`.omop/\` is gitignored - state written during worktree execution would otherwise be lost.
   \`\`\`bash
   cp -r <worktree-path>/.omop/* <main-repo>/.omop/ 2>/dev/null || true
   \`\`\`
