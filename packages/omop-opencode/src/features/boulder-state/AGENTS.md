...
| `storage.ts` | Atomic CRUD on `.omop/boulder.json`. Writes via temp file + rename; file lock per work_id |
...
```
<worktree-root>/.omop/boulder.json   # gitignored; one file per worktree
```
