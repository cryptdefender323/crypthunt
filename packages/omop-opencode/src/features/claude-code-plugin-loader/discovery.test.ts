...
describe("discoverInstalledPlugins", () => {
  beforeEach(() => {
    ...
    const pluginsHome = createTemporaryDirectory("omop-claude-plugins-")
  ...
  it("preserves scoped package name from npm plugin keys", async () => {
    ...
    const installPathBase = createTemporaryDirectory("omop-scoped-plugin-")
  ...
  it("derives package name from file URL plugin keys", async () => {
    //#given
    const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
    const installPath = createTemporaryDirectory("omop-fileurl-plugin-")
    ...
    writeFileSync(
      ...
      JSON.stringify({
        version: 2,
        plugins: {
          "file:///D:/configs/user-configs/.config/opencode/node_modules/crypthunter@latest": [
            {
              scope: "user",
              installPath,
              version: "3.10.0",
    ...
    expect(discovered.errors).toHaveLength(0)
    expect(discovered.plugins).toHaveLength(1)
    expect(discovered.plugins[0]?.name).toBe("crypthunter")
  })

  it("derives canonical package name from npm plugin keys", async () => {
    //#given
    const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
    const installPath = createTemporaryDirectory("omop-npm-plugin-")
  ...
  describe("#given project-scoped entries in v1 format", () => {
    it("#when cwd matches projectPath #then the plugin loads", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("omop-v1-project-match-")
      const installPath = createInstallPath("omop-v1-install-")
    ...
    it("#when cwd is a subdirectory of projectPath #then the plugin loads", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("omop-v1-project-sub-")
    ...
    it("#when cwd does not match projectPath #then the plugin is skipped", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("omop-v1-project-miss-")
      const otherDirectory = createTemporaryDirectory("omop-v1-other-")
      const installPath = createInstallPath("omop-v1-install-")
      writeDatabase(pluginsHome, {
        version: 1,
        plugins: {
    ...
    it("#when scope is user #then it always loads regardless of cwd", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const unrelatedDirectory = createTemporaryDirectory("omop-v1-unrelated-")
      const installPath = createInstallPath("omop-v1-install-")
      writeDatabase(pluginsHome, {
        version: 1,
        plugins: {
  ...
  describe("#given project and local scoped entries in v2 format", () => {
    it("#when cwd matches project-scoped projectPath #then it loads while non-matching entries are dropped", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("omop-v2-project-")
      const otherDirectory = createTemporaryDirectory("omop-v2-other-")
      const matchingInstall = createInstallPath("omop-v2-match-install-")
      const missingInstall = createInstallPath("omop-v2-miss-install-")
      const userInstall = createInstallPath("omop-v2-user-install-")
      writeDatabase(pluginsHome, {
        version: 2,
        plugins: {
    ...
    it("#when scope is local and cwd matches projectPath #then it loads", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("omop-v2-local-match-")
      const installPath = createInstallPath("omop-v2-local-install-")
      writeDatabase(pluginsHome, {
        version: 2,
        plugins: {
          "local-plugin@market": [
    ...
    it("#when scope is local and cwd does not match projectPath #then it is skipped", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("omop-v2-local-miss-")
      const otherDirectory = createTemporaryDirectory("omop-v2-local-other-")
      const installPath = createInstallPath("omop-v2-local-install-")
      writeDatabase(pluginsHome, {
        version: 2,
        plugins: {
    ...
    it("#when multiple installations are present #then only the first is considered and scope filtering still applies", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("omop-v2-multi-")
      const otherDirectory = createTemporaryDirectory("omop-v2-multi-other-")
      const primaryInstall = createInstallPath("omop-v2-multi-primary-")
      const secondaryInstall = createInstallPath("omop-v2-multi-secondary-")
      writeDatabase(pluginsHome, {
        version: 2,
        plugins: {
          "multi-plugin@market": [
  ...
  describe("#given project and local scoped entries in v3 flat-array format", () => {
    it("#when cwd matches projectPath #then projectPath flows through and the plugin loads", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("omop-v3-match-")
      const installPath = createInstallPath("omop-v3-install-")
      writeDatabase(pluginsHome, [
        {
    ...
    it("#when cwd does not match projectPath #then the plugin is skipped", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("omop-v3-miss-")
      const otherDirectory = createTemporaryDirectory("omop-v3-miss-other-")
      const installPath = createInstallPath("omop-v3-install-")
      writeDatabase(pluginsHome, [
        {
        ...
        },
        {
          ...
          marketplace: "market",
          scope: "user",
          installPath: createInstallPath("omop-v3-user-install-"),
          version: "2.0.0",
          lastUpdated: "2026-03-25T00:00:00Z",
        },
      ])
  ...
  describe("#given enabledPluginsOverride combined with scope filtering", () => {
    it("#when a project-scoped plugin is disabled via override #then it is still skipped even if cwd would match", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("omop-enabled-proj-")
      const installPath = createInstallPath("omop-enabled-install-")
      writeDatabase(pluginsHome, {
        version: 2,
        plugins: {
          "gated-plugin@market": [
    ...
    it("#when a project-scoped plugin is enabled and cwd matches #then it loads", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const projectDirectory = createTemporaryDirectory("omop-enabled-match-")
      const installPath = createInstallPath("omop-enabled-match-install-")
      writeDatabase(pluginsHome, {
        version: 2,
        plugins: {
  ...
  describe("#given installed_plugins.json points to a stale version directory", () => {
    ...
    it("#when configured installPath ends in 'unknown' but a sibling version dir has a plugin manifest #then it is recovered without an error", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const cacheRoot = createTemporaryDirectory("omop-cc-plus-cache-")
    ...
    it("#when configured installPath is missing AND no sibling has a plugin manifest #then the original 'path does not exist' error is preserved", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const cacheRoot = createTemporaryDirectory("omop-no-manifest-cache-")
    ...
    it("#when only an 'unknown' sibling exists with a manifest #then it is still picked rather than reporting an error", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const cacheRoot = createTemporaryDirectory("omop-only-unknown-cache-")
    ...
    it("#when the recovered version dir uses the legacy root-level plugin.json layout #then it is recognized and the manifest is loaded", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const cacheRoot = createTemporaryDirectory("omop-legacy-manifest-cache-")
    ...
    it("#when the configured installPath exists #then it is used as-is without scanning siblings", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const cacheRoot = createTemporaryDirectory("omop-existing-path-cache-")
    ...
    it("#when multiple non-'unknown' semver siblings are present #then the highest version is picked deterministically", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const cacheRoot = createTemporaryDirectory("omop-multi-version-cache-")
    ...
    it("#when a sibling directory exists with a manifest whose 'name' does NOT match the plugin key #then it is rejected and the error surfaces", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const cacheRoot = createTemporaryDirectory("omop-wrong-name-cache-")
    ...
    it("#when two siblings share the same X.Y.Z prefix but one is a prerelease #then the plain version wins deterministically", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const cacheRoot = createTemporaryDirectory("omop-prerelease-cache-")
    ...
    it("#when a sibling has a malformed manifest that cannot be parsed #then it is rejected under strict name-match", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const cacheRoot = createTemporaryDirectory("omop-malformed-cache-")
    ...
    it("#when a sibling's manifest lacks a 'name' field #then it is rejected under strict name-match", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const cacheRoot = createTemporaryDirectory("omop-noname-cache-")
    ...
    it("#when installation.version is an empty string and manifest.version is also empty #then resolvedVersion falls back to 'unknown' not ''", async () => {
      //#given
      const pluginsHome = process.env.CLAUDE_PLUGINS_HOME as string
      const cacheRoot = createTemporaryDirectory("omop-empty-version-cache-")
