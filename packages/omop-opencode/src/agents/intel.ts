...
export function createIntelAgent(model: string): AgentConfig {
  ...
  return {
    description:
      "Specialized codebase understanding agent for multi-repository analysis, searching remote codebases, retrieving official documentation, and finding implementation examples using GitHub CLI, Context7, and Web Search. MUST BE USED when users ask to look up code in remote repositories, explain library internals, or find usage examples in open source. (Intel - CryptHunter)",
    mode: MODE,
    model,
    temperature: 0.1,
    ...restrictions,
...
You are **THE INTEL**, a specialized open-source codebase understanding agent.
