...
const OMOP_ATTACH_PANE_TITLE_PREFIXES = ["omop-subagent-", "omop-team-"]
...
async function listTmuxPanesViaTmux(tmux: string): Promise<TmuxAttachPane[]> {
	...
	const result = await runTmuxCommand(tmux, [
		...
		"-a",
		"-F",
		`#{pane_id}\t#{pane_title}\t#{${OMOP_ATTACH_SERVER_URL_OPTION}}\t#{pane_current_command} #{pane_start_command}`,
	])
...
function isOmoAttachPane(pane: TmuxAttachPane): boolean {
	return pane.attachServerUrl.length > 0 || OMOP_ATTACH_PANE_TITLE_PREFIXES.some((prefix) => pane.title.startsWith(prefix))
}
