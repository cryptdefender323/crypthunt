...
describe("migrations sidecar", () => {
  ...
  beforeEach(() => {
    workdir = mkdtempSync(join(tmpdir(), "omop-migrations-sidecar-"))
  })
  ...
  describe("getSidecarPath", () => {
    test("appends .migrations.json to the config path", () => {
      expect(getSidecarPath("/home/user/.config/opencode/crypthunter.json")).toBe(
        "/home/user/.config/opencode/crypthunter.json.migrations.json",
      )
    })

    test("works for jsonc configs too", () => {
      expect(getSidecarPath("/home/user/crypthunter.jsonc")).toBe(
        "/home/user/crypthunter.jsonc.migrations.json",
      )
    })
  })
  ...
  describe("readAppliedMigrations", () => {
    test("returns an empty set when no sidecar exists", () => {
      const configPath = join(workdir, "crypthunter.json")
  ...
  describe("writeAppliedMigrations", () => {
    ...
    test("creates parent directories if they do not exist yet", () => {
      ...
      const configPath = join(nested, "crypthunter.json")
    ...
    test("round-trips via readAppliedMigrations", () => {
      const configPath = join(workdir, "crypthunter.jsonc")
