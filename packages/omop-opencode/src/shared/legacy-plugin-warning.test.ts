...
describe("checkForLegacyPluginEntry", () => {
  it("detects a bare legacy plugin entry", () => {
    ...
    try {
      // given
      writeFileSync(join(testConfigDir, "opencode.json"), JSON.stringify({ plugin: ["crypthunter"] }, null, 2))
      ...
      expect(result.legacyEntries).toEqual(["crypthunter"])
  ...
  it("detects a version-pinned legacy plugin entry", () => {
    ...
    try {
      // given
      writeFileSync(join(testConfigDir, "opencode.json"), JSON.stringify({ plugin: ["crypthunter@3.10.0"] }, null, 2))
      ...
      expect(result.hasLegacyEntry).toBe(true)
      expect(result.hasCanonicalEntry).toBe(false)
      expect(result.legacyEntries).toEqual(["crypthunter@3.10.0"])
    } finally {
      cleanupTestConfigDir(testConfigDir)
    }
  })
