...
describe("sparkshell runtime awareness", () => {
  ...
  test("#given explicit force-on env #when Codex Desktop is absent #then returns Sparkshell guidance", () => {
    ...
    expect(context).toContain("OMOP_SPARKSHELL_BIN")
    expect(context).toContain("OMOP_SPARKSHELL_SESSION_CONTEXT")
    expect(context).toContain("OMOP_SPARKSHELL_CONDENSE")
    expect(context).toContain("OMOP_SPARKSHELL_SPARK")
