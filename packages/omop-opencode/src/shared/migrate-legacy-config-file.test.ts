...
describe("migrateLegacyConfigFile", () => {
  ...
  describe("#given crypthunter.jsonc exists but crypthunter.jsonc does not", () => {
    describe("#when migrating the config file", () => {
      it("#then writes crypthunter.jsonc and renames the legacy file to a backup", () => {
        const legacyPath = join(testDir, "crypthunter.jsonc")
        const backupPath = join(testDir, "crypthunter.jsonc.bak")
        writeFileSync(legacyPath, '{ "agents": {} }')

        const result = migrateLegacyConfigFile(legacyPath)

        expect(result).toBe(true)
        expect(existsSync(join(testDir, "crypthunter.jsonc"))).toBe(true)
        expect(existsSync(legacyPath)).toBe(false)
        expect(existsSync(backupPath)).toBe(true)
        expect(readFileSync(join(testDir, "crypthunter.jsonc"), "utf-8")).toBe('{ "agents": {} }')
  ...
  describe("#given a legacy config sidecar exists", () => {
    describe("#when migrating the config file", () => {
      it("#then copies applied migration history to the canonical sidecar", () => {
        const legacyPath = join(testDir, "crypthunter.json")
        const legacySidecarPath = `${legacyPath}.migrations.json`
        const canonicalSidecarPath = join(testDir, "crypthunter.json.migrations.json")
  ...
  describe("#given crypthunter.json exists but crypthunter.json does not", () => {
    describe("#when migrating the config file", () => {
      it("#then copies to crypthunter.json", () => {
        const legacyPath = join(testDir, "crypthunter.json")
        writeFileSync(legacyPath, '{ "agents": {} }')
        ...
        expect(result).toBe(true)
        expect(existsSync(join(testDir, "crypthunter.json"))).toBe(true)
      })
    })
  })

  describe("#given crypthunter.jsonc already exists", () => {
    describe("#when attempting migration", () => {
      it("#then returns false and does not overwrite", () => {
        const legacyPath = join(testDir, "crypthunter.jsonc")
        const canonicalPath = join(testDir, "crypthunter.jsonc")
      ...
      it("#then does not copy legacy team_mode.tmux_visualization into the canonical file", () => {
        const legacyPath = join(testDir, "crypthunter.json")
        const canonicalPath = join(testDir, "crypthunter.json")
  ...
  describe("#given the file does not exist", () => {
    describe("#when attempting migration", () => {
      it("#then returns false", () => {
        const result = migrateLegacyConfigFile(join(testDir, "crypthunter.jsonc"))
