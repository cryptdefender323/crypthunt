export const SPARKSHELL_USAGE = [
  ...
  "Environment: OMOP_SPARKSHELL_BIN selects the native sidecar path.",
  "When CODEX_THREAD_ID (or OMOP_SPARKSHELL_SESSION_ID) identifies a Codex session, recent session context",
  "is fed to oversized-output condensation for relevance ranking, but is never appended to command output.",
  "OMOP_SPARKSHELL_SESSION_CONTEXT=0 disables that context lookup.",
  "Oversized output is condensed to a budget (default 20000 chars; --budget <chars> or",
  "OMOP_SPARKSHELL_CONDENSE_BUDGET overrides) preserving error signatures, repeated patterns,",
  "session-goal matches, and head/tail. OMOP_SPARKSHELL_CONDENSE=0 disables condensation.",
  ...
  "OMOP_SPARKSHELL_SPARK=0 disables it; OMOP_SPARKSHELL_SPARK_MODEL / OMOP_SPARKSHELL_SPARK_TIMEOUT_MS /",
  "OMOP_SPARKSHELL_SPARK_BIN tune the invocation. Condensation is the automatic fallback.",
].join("\n")
