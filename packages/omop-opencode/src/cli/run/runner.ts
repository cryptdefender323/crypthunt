...
export async function run(options: RunOptions): Promise<number> {
  ...
  // When --mode is supplied, prepend a mode directive so the keyword detector
  // and pentest-context hook pick up the correct engagement mode automatically.
  const message = options.mode
    ? `--mode ${options.mode}\n\n${rawMessage}`
    : rawMessage
