...
| File | Purpose |
|------|---------|
| `add-plugin-to-opencode-config.ts` | Register `crypthunter` in `.opencode/opencode.json` plugin array |
...
| `generate-omop-config.ts` | Generate `crypthunter.jsonc` from install selections |
...
| `plugin-name-with-version.ts` | Resolve `crypthunter@X.Y.Z` for installation |
| `write-omop-config.ts` | Write generated config to `.opencode/crypthunter.jsonc` |
...
5. generate-omop-config (build config from user selections)
6. write-omop-config
