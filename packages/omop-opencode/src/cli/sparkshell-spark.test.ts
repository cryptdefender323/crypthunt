...
describe("sparkshell spark gating", () => {
  test("#given env toggles #when checking enablement #then defaults on and honors falsy kill switches", () => {
    ...
    expect(isSparkSummaryEnabled({ OMOP_SPARKSHELL_SPARK: "1" })).toBe(true)
    for (const falsy of ["0", "false", "no", "off", " OFF "]) {
      expect(isSparkSummaryEnabled({ OMOP_SPARKSHELL_SPARK: falsy })).toBe(false)
    }
  })
  ...
  test("#given model env overrides #when resolving the model #then falls back to the spark default", () => {
    ...
    expect(resolveSparkModel({ OMOP_SPARKSHELL_SPARK_MODEL: "  " })).toBe(DEFAULT_SPARK_MODEL)
    expect(resolveSparkModel({ OMOP_SPARKSHELL_SPARK_MODEL: "gpt-5.4-mini" })).toBe("gpt-5.4-mini")
  })
  ...
  test("#given timeout env overrides #when resolving the timeout #then ignores junk values", () => {
    ...
    expect(resolveSparkTimeoutMs({ OMOP_SPARKSHELL_SPARK_TIMEOUT_MS: "12000" })).toBe(12_000)
    expect(resolveSparkTimeoutMs({ OMOP_SPARKSHELL_SPARK_TIMEOUT_MS: "junk" })).toBe(DEFAULT_SPARK_TIMEOUT_MS)
    expect(resolveSparkTimeoutMs({ OMOP_SPARKSHELL_SPARK_TIMEOUT_MS: "-5" })).toBe(DEFAULT_SPARK_TIMEOUT_MS)
  })
  ...
  test("#given exec arg construction #when a codex profile is configured #then injects --profile and keeps stdin prompt mode", () => {
    ...
    const profiledArgs = buildSparkExecArgs({ OMOP_SPARKSHELL_SPARK_PROFILE: "quotio" }, "/tmp/last-message.txt")
    ...
    expect(buildSparkExecArgs({ OMOP_SPARKSHELL_SPARK_PROFILE: "  " }, "/tmp/last-message.txt")).toEqual(defaultArgs)
  })
})
