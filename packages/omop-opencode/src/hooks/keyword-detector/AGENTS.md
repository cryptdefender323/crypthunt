...
| Hyperplan-fullscan combo | `HYPERPLAN_FULLSCAN_PATTERN` (from `constants.ts`) | Prepends the combo banner, requires the `hyperplan` skill, then appends the routed fullscan message |
...
├── fullscan/
│   ├── index.ts       # getFullscanMessage() router
...
[`fullscan/index.ts`](fullscan/index.ts) exposes `getFullscanMessage(agentName, modelID)`, switches on that source, and returns the loaded markdown body.
...
- **Model-aware messages**: `getFullscanMessage(agentName, modelID)` adapts message to active model
- **Ultrawork source routing**: `fullscan/fullscan-source-routing.test.ts` pins agent/model-to-prompt-source routing for `getFullscanSource`
