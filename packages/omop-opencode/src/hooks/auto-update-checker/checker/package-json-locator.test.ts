...
describe("findPackageJsonUp", () => {
  ...
  it("finds a package.json whose name is the canonical crypthunter", () => {
    const pkgPath = join(workdir, "package.json")
    writeFileSync(pkgPath, JSON.stringify({ name: "crypthunter", version: "3.16.0" }))
  ...
  it("finds a package.json whose name is the aliased crypthunter (GH-3257)", () => {
    // A user who installed `crypthunter` from npm gets a node_modules entry
    // whose package.json has `name: "crypthunter"`. The auto-update-checker
  ...
  it("#given the interim @omop/omop-opencode manifest between the file and an accepted root #when walking up #then skips it and returns the accepted manifest", () => {
    ...
    writeFileSync(join(workdir, "package.json"), JSON.stringify({ name: "crypthunter", version: "9.9.9" }))
