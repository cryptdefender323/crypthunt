...
The single highest-leverage thing Rust gives a pentest agent: encode invariants in the type system so the compiler refuses incorrect code. The agent does not have to "remember" rules - the rules are physical.
