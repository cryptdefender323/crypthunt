...
describe("OpenClaw Dispatcher", () => {
  ...
  test("resolveCommandTimeoutMs reads OMO env fallback", () => {
    const original = process.env.OMOP_OPENCLAW_COMMAND_TIMEOUT_MS
    process.env.OMOP_OPENCLAW_COMMAND_TIMEOUT_MS = "4321"
    ...
    try {
      ...
      if (original === undefined) delete process.env.OMOP_OPENCLAW_COMMAND_TIMEOUT_MS
      else process.env.OMOP_OPENCLAW_COMMAND_TIMEOUT_MS = original
    }
  })
